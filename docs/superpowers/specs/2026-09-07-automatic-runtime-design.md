...
5. Worker workflow makes the runtime-worker boundary explicit; never recursively launch another codeunlimited run. Automatically persisted validated state is the checkpoint. No transcript edits, UI automation, detached infinite watcher, account workarounds, or forced restart of the current desktop chat.
